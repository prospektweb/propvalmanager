...
4. Установщик автоматически заменит поддерживаемые файлы шаблона Аспро Премьер готовыми версиями из `samples-aspro/` и сохранит оригиналы в backup.
...
Установщик модуля выполняет бесшовную интеграцию и заменяет два файла рабочего шаблона Аспро Премьер готовыми версиями из `samples-aspro/`:

| Рабочий файл на сайте | Готовый файл в модуле | Что добавлено |
| --- | --- | --- |
| `/bitrix/templates/aspro-premier/components/bitrix/catalog.element/main/component_epilog.php` | `samples-aspro/bitrix/templates/aspro-premier/components/bitrix/catalog.element/main/component_epilog.php` | Подключение `assets/css/property-description-tooltips.css` и `assets/js/property-description-tooltips.js` через `Bitrix\Main\Page\Asset`, если модуль доступен. |
| `/bitrix/templates/aspro-premier/include/blocks/catalog/props/list.php` | `samples-aspro/include/blocks/catalog/props/list.php` | Вывод `data-pvm-*` атрибутов на `div.properties__value` для сопоставления значений свойств товара с публичным JSON. |
...
Если стандартный файл Аспро не найден, замена этого файла пропускается и записывается в журнал Bitrix. В этом случае проверьте, отличается ли путь шаблона от стандартного.
